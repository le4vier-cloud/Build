  ...
  return (
    <>
      <ModuleLayout title="Stations" subNav={SUB_NAV} activeView={view} onViewChange={setView}>
        {view === "list" && <StationList onAdd={() => setPanelOpen(true)} />}
      </ModuleLayout>

      <RightPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Add New Station">
        <div style={s.form}>
          <div style={s.field}>
          ...
          <div>
            ...
            {workflows.map((w, i) => (
              <div key={i} style={{ ...s.row, marginBottom: 8 }}>
                <input readOnly value={w.productLabel} style={{ ...s.input, width: 160, flexShrink: 0 }} />
                <select value={w.workflowId} onChange={(e) => updateWorkflow(i, e.target.value)} style={s.select}>
...
function StationList({ onAdd }: { onAdd: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>No stations yet.</p>
      <button
        onClick={onAdd}
        style={{
...
const s: Record<string, React.CSSProperties> = {
  ...
  wageLabel:       { fontSize: 13, fontStyle: "italic" as const, color: "var(--text-secondary)" },
  imageBox:        { height: 140, border: "1px solid var(--border)", borderRadius: "var(--radius-md)", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "var(--bg)", cursor: "pointer" },
  imagePlaceholder:{ fontSize: 13, color: "var(--text-secondary)", textAlign: "center" as const, padding: 16 },
