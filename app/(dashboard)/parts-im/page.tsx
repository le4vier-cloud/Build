  ...
  return (
    <>
      <ModuleLayout title="Internally Manufactured — Stock Items" subNav={SUB_NAV} activeView={view} onViewChange={setView}>
        {view === "list" && <PartsList onAdd={() => setPanelOpen(true)} />}
      </ModuleLayout>

      <RightPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="New Stock Item (IM)">
        <div style={s.form}>
          ...
          <Field label="Minimum Threshold" type="number" value={form.min_threshold} onChange={(v) => set("min_threshold", v)} />
          ...
          <Field label='Assembly Task Name *' value={form.assembly_task_name} onChange={(v) => set("assembly_task_name", v)} />

          <div style={s.field}>
            <label style={s.label}>Assembly Task Description</label>
            <textarea value={form.assembly_description}
              onChange={(e) => set("assembly_description", e.target.value)}
              rows={3} style={s.textarea} />
          </div>

          <Field label="Max Produce Time (Minutes)" type="number" value={form.max_produce_minutes} onChange={(v) => set("max_produce_minutes", v)} />
          ...
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <label style={s.label}>OS Stock Items</label>
...
function PartsList({ onAdd }: { onAdd: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <p style={{ color: "var(--text-secondary)", fontSize: 14 }}>No internally manufactured parts yet.</p>
      <button
        onClick={onAdd}
        style={{
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: unknown) => void; type?: string }) {
  return (
    <div style={s.field}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  textarea:    { border: "1px solid var(--input-border)", borderRadius: "var(--radius-sm)", padding: "8px 12px", fontSize: 14, color: "var(--text-primary)", backgroundColor: "var(--surface)", outline: "none", resize: "vertical" as const, fontFamily: "inherit", width: "100%" },
  ...
  toggle:      { width: 44, height: 26, borderRadius: 13, border: "none", cursor: "pointer", position: "relative" as const, transition: "background-color 0.2s", flexShrink: 0 },
  toggleThumb: { position: "absolute" as const, top: 3, width: 20, height: 20, borderRadius: "50%", backgroundColor: "#fff", transition: "transform 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.2)" },
